...
def create_icd_table(table_name, cursor, con):
    # searching for codes table
    if 'codes' in table_name:
        # creating ICD codes table
        stmt = 'CREATE TABLE {} (code VARCHAR(10))'.format(table_name)
        cursor.execute(stmt)
        print "Table is created"

        # dummy list of ICD codes
        mylst = ['571.8', 'K76.0', 'K76.89']
        ...
        cursor.executemany(stmt, [(v,) for v in mylst])
